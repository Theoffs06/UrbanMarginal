...
public class Sound implements Serializable, Runnable {
  ...
  **/
  public Sound(File fichier) throws SonException {
    if(!fichier.exists()) throw new SonIntrouvableException(fichier);
  ...
  private void initialise(File fichier) throws SonException {
	  try {
	      ...
	      DataLine.Info info = new DataLine.Info(Clip.class, this.lecteurAudio.getFormat(), ((int)this.lecteurAudio.getFrameLength() * this.format.getFrameSize()));
    ...
    while(this.thread != null) {
      ...
      if(this.tour > 0) {
        ...
        if(this.tour < 1) {
          ...
          if(this.fermerALaFin) this.fermer();
        }
      }
    }
  }
  ...
  public void retireEcouteurSon(EcouteurSon ecouteur) {
    if(ecouteur != null) this.ecouteurs.removeElement(ecouteur);
  }
